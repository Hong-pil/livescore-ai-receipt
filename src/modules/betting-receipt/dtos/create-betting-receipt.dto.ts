...
export class SimpleGameDto {
  ...
  @ApiProperty({ example: 'B', description: '경기 상태' })
  @IsOptional()
  @IsString()
  state?: string;
...
export class TotoCalcBookmarkDetailDto {
  ...
  @ApiProperty({ example: 'W', description: '예측 결과 (W:승, D:무, L:패, U:언더, O:오버)' })
  @IsString()
  ...
  @ApiProperty({ example: 'H', description: '배팅 타입 (H:핸디캡, U:언더오버, B:승무패)', required: false })
  @IsOptional()
  @IsString()
  type_sc?: string;

  @ApiProperty({ example: '12.05', description: '기준점', required: false })
  @IsOptional()
  @IsString()
...
export class TotoCalcBookmarkRoundInfoDto {
  @ApiProperty({ example: 'P', description: '배당 구분' })
  @IsString()
  diviedend_sc: string;
  ...
  @ApiProperty({ description: '선택한 경기들의 배팅 상세 정보', type: [TotoCalcBookmarkDetailDto] })
  @IsArray()
  @ArrayMinSize(1)
...
export class CreateBettingReceiptDto {
  ...
  @ApiProperty({ example: 'P', description: '베팅 타입', enum: ['P', 'F', 'A', 'B'] })
  @IsEnum(['P', 'F', 'A', 'B'])
  type: string;

  @ApiProperty({ description: '회차 정보', type: RoundInfoDto })
  @IsObject()
  round_info: RoundInfoDto;

  @ApiProperty({ description: '사용자가 선택한 배팅 정보', type: TotoCalcBookmarkRoundInfoDto })
  @IsObject()
  @ValidateNested()
  @Type(() => TotoCalcBookmarkRoundInfoDto)
  calc_model: TotoCalcBookmarkRoundInfoDto;

  @ApiProperty({ description: '선택된 게임 원본 데이터 목록', type: [SimpleGameDto] })
  @IsArray()
  @ArrayMinSize(1)
  @ValidateNested({ each: true })
  @Type(() => SimpleGameDto)
  selected_games: SimpleGameDto[];

  @ApiProperty({ example: '2025-09-11T05:30:00.000Z', description: '생성 일시' })
  @IsString()
  create_date: string;
}
...
export class BettingReceiptFilterDto {
  ...
  @ApiProperty({ example: '2025-09-11', description: '시작 날짜', required: false })
  @IsOptional()
  @IsString()
  start_date?: string;

  @ApiProperty({ example: '2025-09-11', description: '종료 날짜', required: false })
  @IsOptional()
  @IsString()
  end_date?: string;
}
